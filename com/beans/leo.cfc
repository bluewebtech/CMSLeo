<cfcomponent displayname="Leo Bean" 
	output="false" 
	hint="">
	...
	<cffunction name="getUrl" access="public" returntype="string" output="false" 
		hint="Url getter">
		<cfif len(variables.url) eq 0>
			<cfset variables.url = lcase(replace(variables.menu," ","-","all")) />
		<cfelse>
			<cfset variables.url = lcase(replace(variables.url," ","-","all")) />
		</cfif>
